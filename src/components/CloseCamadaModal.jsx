    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className={`${darkMode ? 'dark ' : ''}bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md mx-4`}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-800 dark:text-white">Cerrar Camada</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
            <FontAwesomeIcon icon={faTimes} />
          </button>
        </div>
        <p className="mb-6 text-gray-700 dark:text-gray-300">
          ¿Desea cerrar la camada "{camada.nombre_camada}"? Esta acción desvinculará sus dispositivos.
        </p>
        <div className="flex justify-end space-x-2">
          <button onClick={onClose} className="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded-md">Cancelar</button>
          <button onClick={handleClose} className="px-4 py-2 bg-green-600 text-white rounded-md">
            <FontAwesomeIcon icon={faDoorClosed} className="mr-2" />Cerrar
          </button>
        </div>
      </div>
    </div>
