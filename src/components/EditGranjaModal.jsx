...
export default function EditGranjaModal({ isOpen, onClose, onGranjaUpdated, granja }) {
  ...
  useEffect(() => {
    if (isOpen && granja) {
      setFormData({
        ...
        codigo_postal: granja.codigo_postal || '',
        empresa_id: granja.empresa_id || '',
        numero_rega: granja.numero_rega || '',
        numero_naves: granja.numero_naves || 1,
        lat: granja.lat || '0',
        lon: granja.lon || '0',
      ...
      const cargarTodosUsuarios = async () => {
        try {
          ...
          do {
            response = await UsuarioApiService.getUsuarios(page);
            todosUsuarios = [...todosUsuarios, ...response.data];
            page++;
          } while (page <= response.last_page);

          setUsuarios(todosUsuarios.filter(u => u.alta === 1));
        } catch (error) {
          setUsuarios([]);
        }
      };
