...
class PesoPavosButpremiumApiService {
    /**
     * Obtiene todos los pesos de referencia para pavos But Premium
     */
    async getPesosReferencia() {
        const response = await BaseApiService.get(
            `${API_URL}/api/pavos-butpremium`
        );
    ...
    async getPesoByEdad(edad) {
        const response = await BaseApiService.get(
            `${API_URL}/api/pavos-butpremium/edad/${edad}`
        );
        return response.data;
    }
    ...
    async getPesosByRangoEdad(edadInicio, edadFin) {
        const response = await BaseApiService.post(
            `${API_URL}/api/pavos-butpremium/rango`,
            { 
    ...
    async updatePeso(id, pesoData) {
        const response = await BaseApiService.put(
            `${API_URL}/api/pavos-butpremium/${id}`,
            pesoData
        );
        return response.data;
    }
...
export default new PesoPavosButpremiumApiService();