...
class PesoPavosNicholasselectApiService {
    /**
     * Obtiene todos los pesos de referencia para pavos Nicholas Select
     */
    async getPesosReferencia() {
        const response = await BaseApiService.get(
            `${API_URL}/api/pavos-nicholasselect`
        );
    ...
    async getPesoByEdad(edad) {
        const response = await BaseApiService.get(
            `${API_URL}/api/pavos-nicholasselect/edad/${edad}`
        );
        return response.data;
    }
    ...
    async getPesosByRangoEdad(edadInicio, edadFin) {
        const response = await BaseApiService.post(
            `${API_URL}/api/pavos-nicholasselect/rango`,
            { 
    ...
    async updatePeso(id, pesoData) {
        const response = await BaseApiService.put(
            `${API_URL}/api/pavos-nicholasselect/${id}`,
            pesoData
        );
        return response.data;
    }
...
export default new PesoPavosNicholasselectApiService();