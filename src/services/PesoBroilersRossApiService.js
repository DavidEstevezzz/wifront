// src/services/PesoBroilersRossApiService.js
import BaseApiService from './BaseApiService';

const API_URL = import.meta.env.VITE_API_URL || 'https://api.wicontrol.site';

class PesoBroilersRossApiService {
  /**
   * Obtiene todos los datos de referencia de peso para la estirpe Ross
   */
  /**
   * Obtiene todos los datos de referencia de peso para la estirpe Ross
   */
  async getPesosReferencia() {
    try {
      const response = await BaseApiService.get(
        `${API_URL}/api/peso-ross`
      );
      return response.data;
    } catch (error) {
      console.error('Error obteniendo pesos de referencia Ross:', error);
      throw error;
    }
  }
  ...
  async getPesoReferenciaByEdad(edad) {
    try {
      ...
      const response = await BaseApiService.get(
        `${API_URL}/api/peso-ross/edad/${edad}`
      );
      return response.data;
    } catch (error) {
      console.error(`Error obteniendo peso Ross para edad ${edad}:`, error);
      throw error;
    }
  }
  ...
  async getPesosReferenciaByRango(edadInicial, edadFinal) {
    try {
      ...
      console.error(`Error obteniendo pesos Ross para rango ${edadInicial}-${edadFinal}:`, error);
      throw error;
    }
  }
...
export default new PesoBroilersRossApiService();